-- ============================================================
-- 【2回目】RLS（Row Level Security）ポリシー設定（v2_B：会員機能なし＋注文照会あり版）
...
-- ─── cart_items ──────────────────────────────────────
-- ※ v2_Bでは未使用だが、将来の会員機能用にポリシーを設定しておく
...
-- ※ ゲストの注文照会は RLS を直接通さず、
--   SECURITY DEFINER 関数（lookup_guest_order）経由で安全に取得する
