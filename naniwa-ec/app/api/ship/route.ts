// 発送処理API：注文ステータスの更新と発送通知メールの送信を実行します
...
export async function POST(request: NextRequest) {
  ...
  // 2. 管理画面で入力された「注文ID・配送業者・追跡番号」の確認
  ...
  // テスト用送信先があれば優先（開発・テスト用）、なければ注文者のアドレスを使用
